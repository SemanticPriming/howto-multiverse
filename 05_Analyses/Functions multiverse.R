...
stackedBar <- function(prop, main = "Proportion Bar", col = hcl.colors(length(prop), palette = "mint"), labels = NULL, posLegend = c(0, 0), margins = c(2.5, 4, 2.5, 10) + 0.1) {
  # Plot area setup without axis labels and box
  par(mar = margins) 
  plot(1, type = "n", xlim = c(0, 1), ylim = c(0.3, 0.7), xaxt = 'n', yaxt = 'n', xlab = "", ylab = "", main = main, bty = "n", cex.main = 0.8)
  ...
  for (i in seq_along(prop)) {
    rect(start, 0.45, start + prop[i], 1, col = col[i], border = "white")
  ...
  if (!is.null(labels)) {
    par(xpd = TRUE)
    legend("topright", inset = posLegend, legend = labels, fill = col, title = "Category", cex = 0.8, bty = "n")
    par(xpd = FALSE)
  }
}
